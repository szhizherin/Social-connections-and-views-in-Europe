...
# Outputs: predict_SCI/SCI_and_sum_preferences.csv
# Дата: 2021-08-24
...
SCI_and_sum_preferences <- SCI_and_preferences %>% 
  distinct(across(colnames(SCI_and_preferences)[3:dim(SCI_and_preferences)[2]]), .keep_all = TRUE)


# сохраним результат
write.csv(SCI_and_sum_preferences, "predict_SCI/SCI_and_sum_preferences.csv")
