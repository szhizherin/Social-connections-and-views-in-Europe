...
SCI_matrix <- xlsx_matrix_to_R("intermediate_data/SCI_matrix_wvs.xlsx")
SCI_W <- SCI_matrix / apply(SCI_matrix, 1, sum) # нормировка (сумма по строке = 1)

# здесь должны быть величины, обратные расстояниям
distance_matrix <- xlsx_matrix_to_R("intermediate_data/distance_matrix_wvs.xlsx")
...
sq_distance_matrix <- xlsx_matrix_to_R("intermediate_data/distance_matrix_wvs.xlsx")
...
formul_long <- Conf_in_courts ~ EU_friends_abroad +
  ED3_4 + ED5_8 + 
...
formul_short <- Conf_in_courts ~ EU_friends_abroad +
  ED5_8 + 
  Y10_19 + Y20_29 + Y30_39 + Y40_49 + Y50_59 + Y60_69 + Y70_MAX + 
...
moran.mc(world_values_survey$Conf_in_courts, mat2listw(SCI_W), nsim = 1000)
moran.mc(world_values_survey$Conf_in_courts, mat2listw(distance_W), nsim = 1000)
moran.mc(world_values_survey$Conf_in_courts, mat2listw(sq_distance_W), nsim = 1000)
