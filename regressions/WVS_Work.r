...
formul_long <- Work ~ EU_friends_abroad +
  ED3_4 + ED5_8 + 
...
formul_short <- Work ~ EU_friends_abroad +
  ED5_8 + 
  Y10_19 + Y20_29 + Y30_39 + Y40_49 + Y50_59 + Y60_69 + Y70_MAX + 
...
moran.mc(world_values_survey$Work, mat2listw(SCI_W), nsim = 1000)
moran.mc(world_values_survey$Work, mat2listw(distance_W), nsim = 1000)
moran.mc(world_values_survey$Work, mat2listw(sq_distance_W), nsim = 1000)
...
SAR_distance_short <- lagsarlm(formul_short, data = world_values_survey, listw = mat2listw(distance_W))
summary(SAR_distance_short, Nagelkerke = TRUE, signif.stars = TRUE)


# прямые и непрямые эффекты
impacts(SAR_distance_long, listw = mat2listw(distance_W))
impacts(SAR_distance_short, listw = mat2listw(distance_W))
...
rob_check_long <- check_results(data = world_values_survey, mat = distance_W, est_function = lagsarlm, 
                                est_formula = formul_long, n_drop_regions = 10, n_iter = 1000)

rob_check_short <- check_results(data = world_values_survey, mat = distance_W, est_function = lagsarlm, 
                                 est_formula = formul_short, n_drop_regions = 10, n_iter = 1000)

# визуализация проверки робастности
mod <- SAR_distance_short
