...
formul_long <- Trust ~ EU_friends_abroad +
  ED3_4 + ED5_8 + 
...
formul_short <- Trust ~ EU_friends_abroad +
  ED5_8 + 
  Y10_19 + Y20_29 + Y30_39 + Y40_49 + Y50_59 + Y60_69 + Y70_MAX + 
...
moran.mc(world_values_survey$Trust, mat2listw(SCI_W), nsim = 1000)
moran.mc(world_values_survey$Trust, mat2listw(distance_W), nsim = 1000)
moran.mc(world_values_survey$Trust, mat2listw(sq_distance_W), nsim = 1000)
...
SAR_SCI_long <- lagsarlm(formul_long, data = world_values_survey, listw = mat2listw(SCI_W))
...
SAR_sq_distance_long <- lagsarlm(formul_long, data = world_values_survey, listw = mat2listw(sq_distance_W))
summary(SAR_sq_distance_long, Nagelkerke = TRUE, signif.stars = TRUE)

# SEM модели с матрицей SCI
SEM_SCI_short <- errorsarlm(formul_short, data = world_values_survey, listw = mat2listw(SCI_W))
summary(SEM_SCI_short, Nagelkerke = TRUE, signif.stars = TRUE)

# SEM модели с матрицей distance
SEM_distance_short <- errorsarlm(formul_short, data = world_values_survey, listw = mat2listw(distance_W))
summary(SEM_distance_short, Nagelkerke = TRUE, signif.stars = TRUE)
...
Hausman.test(SEM_SCI_short)
Hausman.test(SEM_distance_short)
Hausman.test(SEM_sq_distance_short)


# прямые и непрямые эффекты
impacts(SAR_SCI_long, listw = mat2listw(SCI_W))
impacts(SAR_distance_long, listw = mat2listw(distance_W))
impacts(SAR_sq_distance_long, listw = mat2listw(sq_distance_W))


# тестирование лучшей модели на гетероскедастичность (гетероскедастичность присутствует)
bptest.sarlm(SAR_SCI_long)
bptest.sarlm(SAR_distance_long)
bptest.sarlm(SAR_sq_distance_long)
...
rob_check_long_SCI <- check_results(data = world_values_survey, mat = SCI_W, est_function = lagsarlm, 
...
rob_check_long_sq_distance <- check_results(data = world_values_survey, mat = sq_distance_W, est_function = lagsarlm, 
                                 est_formula = formul_long, n_drop_regions = 10, n_iter = 1000)

# визуализация проверки робастности
mod <- SAR_SCI_long
check <- rob_check_long_SCI
...
mod <- SAR_sq_distance_long
check <- rob_check_long_sq_distance
